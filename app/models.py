...
class User(UserMixin, db.Model):
    ...
    groups = db.relationship('UserGroup', secondary=user_group_members, back_populates='members')
    
    # One-to-many: todos assigned directly to this user
    assigned_todos = db.relationship('Todo', back_populates='assigned_user', foreign_keys='Todo.assigned_user_id')
...
class UserGroup(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50), unique=True, nullable=False)  # e.g., "backend-team", "frontend-team", "qa-team"
    description = db.Column(db.String(200), nullable=True)
    
    # Many-to-many: groups can have multiple users
    members = db.relationship('User', secondary=user_group_members, back_populates='groups')
    
    # One-to-many: todos assigned to this group
    assigned_todos = db.relationship('Todo', back_populates='assigned_group', foreign_keys='Todo.assigned_group_id')
...
class Todo(db.Model):
    ...
    date_from = db.Column(db.DateTime, nullable=True)  # Optional: when task should start
    date_to = db.Column(db.DateTime, nullable=True)    # Optional: when task should be done
    ...
    # Relationships
    assigned_user = db.relationship('User', back_populates='assigned_todos', foreign_keys=[assigned_user_id])
    assigned_group = db.relationship('UserGroup', back_populates='assigned_todos', foreign_keys=[assigned_group_id])
